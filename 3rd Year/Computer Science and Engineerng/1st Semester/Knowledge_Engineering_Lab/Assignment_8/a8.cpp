...
int main(){
	...
	vector<string>v;
	print(T,v,100);
	outputfile<<"Conditional Pattern Base "<<"\n";
	for(auto x : CPB){
	...
	for(auto x : CFPT){
		if(x.second >= min_support){
			...
			outputfile<<x.first.first<<"->"<<x.first.second<<"\t-\t"<<x.second<<"\n";
		}
	}
	outputfile<<"Frequent patterns Generated "<<"\n";
	for(auto x : FPG){
		...
		for(int i = 0;i < result.size();i++){
			...
			outputfile<<"\t-\t"<<CFPT[{x.first,result[i][0]}]<<"\n";
		}
	}
}