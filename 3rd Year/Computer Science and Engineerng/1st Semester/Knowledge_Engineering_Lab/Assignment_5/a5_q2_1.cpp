...
int eval(vector<string>ans){
	...
	for(auto x:data_){
		...
		for(int i = 0;i<ans.size();i++){
			if(find(x.second.begin(),x.second.end(),ans[i]) != x.second.end())continue;
			else{
				flag = false;
				break;
			}
...
void printC(map<vector<string> , int > m){
	for(auto x:m){
		for(int i = 0;i<x.first.size();i++) outputfile<<x.first[i]<<" ";
		...
		for(it2 = it1;it2 != apri.end();++it2){
			if(compare(it1->first,it2->first,len) == true){
				...
				for(int i = 0;i<it1->first.size();i++){
					if(asso1 == "{") asso1+=it1->first[i];
					else	asso1+=","+it1->first[i];
...
int main(){
	ifstream inputfile;
	outputfile.open("Output_2_1.txt");
	inputfile.open("Input_2.txt");
	string s;
	string temp = "";
	map<vector<string>,int>m;
	while(inputfile>>s){
		if(s.size() >= 2 && s[0] == 'T' && s[1] >= '0' && s[1] <= '9') temp = s;
		else{
			data_[temp].push_back(s);
			m[{s}]++;
		}
	}
	int n = m.size();
	int A[] = {30,50,70};
	for(int i = 0;i<3;i++){
		min_support = floor((A[i]*data_.size())/100);
