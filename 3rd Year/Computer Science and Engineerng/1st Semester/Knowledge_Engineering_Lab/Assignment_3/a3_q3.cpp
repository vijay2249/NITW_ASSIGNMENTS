...
int main(){
  ...
  while(t<=10){
    ...
    TRX += to_string(t);
    random_device r;
    mt19937_64 generator(r());
    uniform_int_distribution<ll> dist1{1,m};
    uniform_int_distribution<ll> dist2{1,7};
    set<ll> result;
    ll amount = dist2(generator);
    while(result.size() != amount){
      result.insert(dist(generator));
    }
    for(auto i: result){
      trx[TRX].insert("I"+to_string(i));
      counts["I"+to_string(i)]++;
    }
    size_max = max(size_max, (ll)trx[TRX].size());
    t++;
  }
  output<<"Generated transaction database : \n";
  for(auto i : trx){
    output<<i.first<<"\n";
    for(auto j: i.second) output<<j<<"\n";
    output<<"\n";
  }
  output<<"\n\n";
